use crate::scrapers::{create_magnet, extract_info_hash, ScrapedTorrent, Scraper};
use anyhow::Result;
...
impl NekoBtScraper {
    ...
    async fn search_api(&self, query: &str, api_key: &str) -> Result<Vec<ScrapedTorrent>> {
        ...
        for torrent in api_data.torrents {
            ...
            let magnet_link = torrent
                .magnet
                .unwrap_or_else(|| create_magnet(&info_hash, &torrent.title));
    ...
    async fn search_html(&self, query: &str) -> Result<Vec<ScrapedTorrent>> {
        ...
        for selector_str in &row_selectors {
            ...
            for row in document.select(&row_selector) {
                ...
                let title_elem = row.select(&title_selector).find(|a| {
                    a.value()
                        .attr("href")
