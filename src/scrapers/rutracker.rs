...
impl RuTrackerScraper {
    pub fn new() -> Result<Self> {
        let session_cookie = std::env::var("RUTRACKER_COOKIE")
            .unwrap_or_default();
    ...
    async fn search_rutracker(&self, query: &str) -> Result<Vec<ScrapedTorrent>> {
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!("RuTracker returned status: {}", response.status()));
        }
        ...
        for row in document.select(&row_selector) {
            ...
            let topic_url = title_links.first()
                .and_then(|a| a.value().attr("href"))
                .unwrap_or("");
            
            let topic_id = topic_url
                .split('=')
                .nth(1)
                .unwrap_or("");
            ...
            let magnet_link = format!("magnet:?xt=urn:btih:{}&dn={}", info_hash, urlencoding::encode(&title));
