use hydrogene::scrapers::{ScraperManager, ScrapedTorrent, Scraper};
...
async fn main() -> anyhow::Result<()> {
    ...
    for (content_type, id, title, episode_info) in test_cases {
        ...
        let search_queries = if id.starts_with("anilist:") {
            ...
            match metadata_client.resolve_anime_title(&format!("anilist:{}", anime_id)).await {
                Some(anime_title) => {
        ...
        let mut results_4k: Vec<ScrapedTorrent> = quality_torrents
            ...
            .filter(|t| {
                let title_upper = t.title.to_uppercase();
                title_upper.contains("2160P") || title_upper.contains("4K") || title_upper.contains("UHD")
            })
            .cloned()
            .collect();
        ...
        total_results.insert(title.to_string(), (total_count, results_4k.len(), results_1080p.len()));
        
        // Display results by quality
        if !results_4k.is_empty() {
            println!("\n🏆 4K / 2160p RESULTS ({} found):", results_4k.len());
            println!("{:<5} {:<60} {:<12} {:<15} {}", "#", "Title", "Size", "Seeders", "Source");
            println!("{}", "-".repeat(110));
    ...
    for (title, (total, count_4k, count_1080p)) in &total_results {
        println!("{:<30} {:<12} {:<12} {}", title, total, count_4k, count_1080p);
    }
    ...
    for provider in providers {
        if let Some((count, working)) = provider_status.get(provider) {
            let status = if *working { "✅ WORKING" } else { "❌ FAILED" };
