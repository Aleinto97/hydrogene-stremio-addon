...
async fn main() -> anyhow::Result<()> {
    ...
    // Get table names
    let rows: Vec<(String,)> = sqlx::query_as("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'")
        .fetch_all(&pool)
        .await?;
    ...
    let deleted = sqlx::query("DELETE FROM cached_torrents")
        .execute(&pool)
        .await?;
    println!("\n🗑️ Deleted {} rows from cached_torrents", deleted.rows_affected());
    
    // Delete from cached_resolves
    let deleted = sqlx::query("DELETE FROM cached_resolves")
        .execute(&pool)
        .await?;
    println!("🗑️ Deleted {} rows from cached_resolves", deleted.rows_affected());
