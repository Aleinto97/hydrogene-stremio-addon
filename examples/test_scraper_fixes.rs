...
async fn main() -> anyhow::Result<()> {
    ...
    match tokio::time::timeout(
        ...
        nyaa.search("Attack on Titan", "series"),
    )
    .await
    {
    ...
    match tokio::time::timeout(Duration::from_secs(10), rutor.search("Inception", "movie")).await {
        Ok(Ok(results)) => {
            ...
            if let Some(first) = results.first() {
                println!(
                    "      Primo: '{}' - Seeders: {}",
                    first.title.chars().take(50).collect::<String>(),
                    first.seeders
                );
            }
        }
    ...
    match tokio::time::timeout(
        Duration::from_secs(10),
        rutracker.search("Inception", "movie"),
    )
    .await
    {
        Ok(Ok(results)) => println!(
            "   ✅ RuTracker: {} risultati (cookie: {})",
            results.len(),
            if std::env::var("RUTRACKER_COOKIE").is_ok() {
                "presente"
            } else {
                "non configurato"
            }
    ...
    match tokio::time::timeout(
        Duration::from_secs(10),
        bitsearch.search("Inception", "movie"),
    )
    .await
    {
    ...
    match tokio::time::timeout(
        Duration::from_secs(10),
        eztv.search("Breaking Bad", "series"),
    )
    .await
    {
    ...
    match tokio::time::timeout(
        Duration::from_secs(10),
        nekobt.search("Attack on Titan", "series"),
    )
    .await
    {
