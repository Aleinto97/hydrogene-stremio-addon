use hydrogene::scrapers::{Scraper, ScrapedTorrent};
...
async fn main() -> anyhow::Result<()> {
    ...
    for (query, content_type, label) in test_queries {
        ...
        while let Some((provider_name, results, _)) = rx.recv().await {
            ...
            println!("  [{:>5.2}s] {}: {} torrent trovati", 
                elapsed, provider_name, results.len());
        }
        ...
        for (name, (time, count)) in sorted_providers {
            ...
            println!("  {:<15} - {:>5.2}s - {:>3} torrent {}", name, time, count, speed);
        }
        ...
        println!("  Provider che hanno risposto entro 1s: {}/{}", completed_in_1s, provider_times.len());
        println!("  Provider che hanno risposto entro 2s: {}/{}", completed_in_2s, provider_times.len());
        println!("  Provider che hanno risposto entro 5s: {}/{}", completed_in_5s, provider_times.len());
