...
async fn main() -> anyhow::Result<()> {
    ...
    let test_cases = vec![("movie", "tt1375666", "Inception", "")];
    ...
    for (content_type, _id, title, _episode_info) in test_cases {
        ...
        let torrents_to_check: Vec<ScrapedTorrent> =
            quality_torrents.into_iter().take(10).collect();

        println!("\n🔍 BATCH CACHE CHECK");
        println!("{}", "=".repeat(50));
        println!(
            "Checking {} torrents on Real-Debrid...",
            torrents_to_check.len()
        );
        ...
        let cached_torrents = match debrid_client.check_batch_cache(&torrents_to_check).await {
            Ok(torrents) => {
                ...
                println!(
                    "{:<5} {:<12} {:<8} {:<50}",
                    "#", "Status", "Seeders", "Title"
                );
                println!("{}", "-".repeat(80));

                for (i, t) in torrents.iter().take(10).enumerate() {
                    let status = if t.is_cached {
                        "✅ CACHED"
                    } else {
                        "❌ NOT CACHED"
                    };
        ...
        let cached: Vec<&ScrapedTorrent> = cached_torrents.iter().filter(|t| t.is_cached).collect();
